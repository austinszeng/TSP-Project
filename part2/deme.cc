...
// fix: you overwrite the population by assigning the new population to the pop_ variable without 
// deleting the previous population; this causes a big memory leak with each population.
{
  ...
  for (i=0; i < pop_2.size(); i++){
    delete (pop_2[i]); // delete previous Chromosome* in population vector
  }
  pop_2.clear()
